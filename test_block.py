...
class TestBlockHeader(unittest.TestCase):    
    def test_blk_hdr_size(self):
        blk_hdr = BlockHeader()
        self.assertTrue(blk_hdr.struct is not None, 'Block header must have a struct defined')
        self.assertEqual(blk_hdr.struct.size, 77, 'Block header struct size must be 77 bytes')
    ...
    def test_blk_hdr_serialize(self):
        ...
        self.assertEqual(serial_buf, expected_buf, 'Serialized blk hdr contents not match')
    ...
    def test_blk_hdr_deserialze(self):
        ...
        for k, _ in temp_blk_hdr.__dict__.items():
            self.assertTrue(temp_blk_hdr.__dict__[k] == blk_hdr.__dict__[k], 'Block header changed after deserialize')
    ...
    def test_blk_hdr_hash(self):
        ...
        self.assertNotEqual(wrong_hash, blk_hdr.CalculateHash(), 'Do not include hash field')
        
        packStructNoHash = struct.Struct(">Bd32sI")
        buf = packStructNoHash.pack(blk_hdr.version, blk_hdr.time, blk_hdr.hashPrevBlock, blk_hdr.nonce)
...
class TestBlock(unittest.TestCase):
    def test_block_format(self):
        blk = Block()
        self.assertTrue(blk.struct is not None, 'Block header must have a struct defined')
    ...
    def test_block_mine(self):
        blk = Block()
        self.assertTrue(blk.MineBlock().startswith(b'\x00' * BLK_DIFFICULTY), 'Did not mine according to difficulty')
    ...
    def test_block_valid(self):
        ...
        self.assertFalse(blk.IsBlockValid(check_sig=True), 'Hash not set yet, should be false')
        ...
        blk.signature += b'0'
        self.assertFalse(blk.IsBlockValid(check_sig=True), 'Signature not matching, should be false')
